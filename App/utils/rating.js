 ...
 */
const Rating = props => {
	const { rate = 3, width: fullwidth, style, rateAble, children, id } = props;
	...
	return (
		<View style={[styles.container, { width: fullwidth }, style]}>
			{rateArray.map((value, _i) =>
				rateAble ? (
					<TouchableOpacity key={_i} onPressOut={e => handleRating(_i, e)}>
						<FontAwesome solid name={face} size={faceWidth} color={value === 1 ? "#ffd700" : "#cecece"} />
					</TouchableOpacity>
				) : (
					<FontAwesome solid key={_i} name={face} size={faceWidth} color={value === 1 ? "#ffd700" : "#cecece"} />
				)
			)}
			{children}
		</View>
